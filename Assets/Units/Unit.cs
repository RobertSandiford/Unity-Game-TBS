...
public class Unit : MonoBehaviour
{
    ...
    public void Init0()
    {
        ...
        cargo = new Cargo(unitDef.cargoDef);
        ...
        if (unitDef.minAltitude > 0)
            altitude = Math.Max(unitDef.minAltitude, map.GetMinTileAlt(tile));
        else
            altitude = 0;
    ...
    public void AssignUnitDef(UnitDef unitDef)
    {
        unitDef = ObjectExtensions.Copy(unitDef);
    }
    ...
    public void MoveTo(Tile to, int newAltitude = 0)
    {
        ...
        ConsumeActionPoint(); //Debug.Log("Consuming action point on " + unitDef.name + " for moving");
        MoveToCurrentTile();
    }
    ...
    public void LoadInto(Unit carrierUnit)
    {
        ...
        ConsumeActionPoint();//Debug.Log("Consuming action point on " + unitDef.name + " for loading");
    ...
    public void UpdateCargoText()
    {
        ...
        if (cargo.containedSquads.Count > 0) {
            foreach (Unit unit in cargo.containedSquads)
            {
                cargoUnits.Add(unit.unitDef.shortName);
            }
    ...
    {
        Unit dismountSquad = cargo.containedSquads[0];
    ...
    public void DebusTo(Tile to)
    {
        //map.UnassignUnitFromTile(tile.x, tile.z);
        ExitCargo(to);
        ConsumeActionPoint();//Debug.Log("Consuming action point on " + unitDef.name + " for exiting");
    }
    ...
    void ShowMoveOptions()
    {
        ...
        availableMoves = map.GetMoves(new int[] { tile.x, tile.z }, unitDef.moves);
    ...
    //        ? map.GetMoves(tile, unitDef.moves)
    ...
    public List<Action> GetArialMoveActions()
    {
        ...
        double adjustedAltGainCost = unitDef.gainAltCost / map.baseHeightMultiplier;
        double adjustedLoseAltPayment = unitDef.loseAltPayment / map.baseHeightMultiplier;

        int maxPossibleHeight = currentHeight + (int)Math.Floor((double)unitDef.moves / adjustedAltGainCost);
        ...
        foreach (ArialMove arialMove in options)
        {
            ...
            minHeight = Math.Max(currentHeight - unitDef.moves, minHeight);

            int maxHeight = 0;
            if (dist <= unitDef.moves)
            {
                maxHeight = currentHeight + (int)Math.Floor((double)(unitDef.moves - dist) / adjustedAltGainCost);
            } else
            {
                maxHeight = currentHeight - (int)Math.Ceiling((double)(dist - unitDef.moves) / adjustedLoseAltPayment);
            }
            maxHeight = Math.Min(maxHeight, unitDef.maxAltitude);
    ...
    public List<ArialMove> GetArialMoves()
    {
        ...
        List<ArialMove> moves = map.GetArialMoves(tile, unitDef.moves, altitude, unitDef.gainAltCost, unitDef.loseAltPayment);
    ...
    public List<Tile> GetAvailableLoadOptions(Dictionary<string, Move> moves)
    {
        ...
        //if (unitDef.targetType != TargetType.Infantry) return loadOptions;

        //List<Tile> tiles = map.GetTilesInMoveRange(tile, unitDef.moves);
    ...
    public List<Tile> GetUnloadOptions(Dictionary<string, Move> moves)
    {
        ...
        if (cargo.containedSquads.Count == 0) return new List<Tile>();
    ...
    public bool CanDoPickup()
    {
        ...
        if (unitDef.maxAltitude > 0) return false;
    ...
    public List<Action> GetPickupActions(Dictionary<string, Move> moves)
    {
        ...
        foreach (KeyValuePair<string, Move> moveKvp in moves)
        {
            ...
            {
                if (cargo.transportable.Contains(move.tile.unit.unitDef.unitClass) && cargo.CanLoad(move.tile.unit))
                {
    ...
    public void SelectFriendly()
    {
        ...
        ui.ShowUnitUI(this, unitDef);
        ui.UpdateTargetingPanelThisUnit(unitDef);
    ...
    public void SpottedBy(Unit unit) {
        //Debug.Log(unitDef.name + " spotted");
    ...
    {
        if (unitDef.weapons == null) return 0;

        int range = 0;
        foreach (UnitWeapon uWeapon in unitDef.weapons)
        {
   ...
   public bool CanSee(Tile targetTile)
    ...
    {
        if (unitDef.weapons == null) return false;
        ...
        if ( CanSee(targetUnit) )
        {
            ...
            for (int i = 0; i < unitDef.weapons.Length; i++) {
    
                UnitWeapon uWeapon = unitDef.weapons[i];
    ...
    public bool CanShootTargetWithWeapon(Unit targetUnit, int weaponId, bool showBlocking = false)
    {
        ...
        if ( CanSee(targetUnit) )
        {
            ...
            UnitWeapon uWeapon = unitDef.weapons[weaponId];
    ...
    private bool CanShootTargetWithWeaponAtRange(Unit targetUnit, int gameRange, UnitWeapon uWeapon)
    {
        if (uWeapon.ammo >= 1)
        {
            ...
            {
                if (uWeapon.weapon.damageProfiles.ContainsKey(targetUnit.unitDef.targetType))
                {
                    double expDamage = uWeapon.weapon.damageProfiles[targetUnit.unitDef.targetType].GetAverageDamage(targetUnit, uWeapon, gameRange, targetUnit.tile.terrain, targetUnit.unitDef.countermeasures, Moving());
    ...
    public List<FiringWeapon> SelectFiringWeapons(Unit targetUnit)
    {
        ...
        for (int i = 0; i < unitDef.weapons.Length; i++)
        {
            ...
            if (CanShootTargetWithWeaponAtRange(targetUnit, gameRange, uWeapon))
            {
                double expDamage = uWeapon.weapon.damageProfiles[targetUnit.unitDef.targetType].GetAverageDamagePerWeapon(targetUnit, uWeapon, gameRange, targetUnit.tile.terrain, targetUnit.unitDef.countermeasures, Moving());
                if (expDamage > 0)
                {
        ...
        if (weps.Count > 0) {
            ...
            int soldiersRemaining = unitDef.soldiers;
    ...
    public void FireWeapon(Unit targetUnit, FiringWeapon fWeapon, int range, bool moved)
    {
        UnitWeapon uWeapon = fWeapon.uWeapon;

        DamageProfile damageProfile = uWeapon.weapon.damageProfiles[targetUnit.unitDef.targetType];
    ...
    public void FireWeaponAtTile(Tile targetTile, FiringWeapon fWeapon, int range, bool moved)
    {
        UnitWeapon uWeapon = fWeapon.uWeapon;
        ...
        if ( CanShootTargetWithWeapon(targetUnit, weaponId) )
        {
            ...
            if (uWeapon.weapon.damageProfiles.ContainsKey(targetUnit.unitDef.targetType))
            {
               ...
               //Debug.Log("Shooting - moved? " + moved);
                ...
                ConsumeActionPoint();//Debug.Log("Consuming action point on " + unitDef.name + " for shooting");
                Fired();
    ...
    public void ShootAtTile(Tile targetTile)
    {
        ...
        if (visibleTiles.Contains(targetTile)) {
            ...
            foreach (FiringWeapon fWeapon in weapons)
            {
                FireWeaponAtTile(targetTile, fWeapon, range, moved);
            }
    ...
    {
        return unitDef.weapons[weaponId].ammo;
    }

    public bool HasAmmo(int weaponId, int ammoRequired = 1)
    {
        return unitDef.weapons[weaponId].ammo >= ammoRequired;
    }
    ...
    public void ConsumeAmmo(int weaponId, int ammoUsed = 1)
    {
        ...
        //Debug.Log(weaponId);
        unitDef.weapons[weaponId].ammo = Math.Max(0, unitDef.weapons[weaponId].ammo - ammoUsed);
        //Debug.Log(unitDef.weapons[weaponId].ammo);
    }
    ...
    public void Die()
    {
        ...
        if (cargo.HasCargo())
        {
            int len = cargo.containedSquads.Count;
            for (int i = 0; i < len; i++)
            {
                Unit dismountSquad = cargo.containedSquads[0];
                cargo.UnloadUnit(dismountSquad);
    ...
    public void Fortify()
    {
        if (tile.fort == null)
        {
            Forts.MakeFort(FortDefs.forts[unitDef.fortType], tile);

            //GameObject fortDisc = Instantiate(fortDiscObj);
            //Fort fort = (Fort)fortDisc.GetComponent(typeof(Fort));
            //fort.Setup(FortDefs.forts[unitDef.fortType], tile);
            //tile.fort = fort;
            // //tile.fort = new Fort(FortDefs.forts[unitDef.fortType]);

            buildFortAtTurnEnd = true;
            ConsumeActionPoint();//Debug.Log("Consuming action point on " + unitDef.name + " for fortifying");
        }
    }
    ...
    public void Setup()
    {
        if (HasSetupWeapon())
        {
            if ( isPackedUp && ! isSettingUp )
            {
                ...
                isSettingUp = true;
                setupProgress = 1;
                ConsumeActionPoint();//Debug.Log("Consuming action point on " + unitDef.name + " for setting up");
    ...
    public void ProgressSetup()
    {
        ...
        {
            setupProgress += 1;
            ConsumeActionPoint();//Debug.Log("Consuming action point on " + unitDef.name + " for continuing to set up");
    ...
    public void Packup()
    {
        ...
        if (HasSetupWeapon())
        {
            if (isSetUp && !isPackingUp)
            {
                ...
                isPackingUp = true;
                packupProgress = 1;
                ConsumeActionPoint(); //Debug.Log("Consuming action point on " + unitDef.name + " for packing up");
                global.ui.HidePackupButton();
                global.ui.HideArtilleryButton();
                global.playerInput.EndArtilleryTargetingMode();
            }
        }
    }
    ...
    public void ProgressPackup()
    {
        ...
        else
        {
            ...
            ConsumeActionPoint(); //Debug.Log("Consuming action point on " + unitDef.name + " for continuing pack up");
        }
    }
    ...
    public void ArtilleryAttack(UnitWeapon uWeapon, ArtilleryTarget target, int shots)
    {
        ...
        ConsumeActionPoint();//Debug.Log("Consuming action point on " + unitDef.name + " for arty start");
    }
    ...
    {
        return unitDef.moves - Math.Max(0,altChange);
    }
